...
static func draw_cell(
	...
	var progress_color := _get_interpolated_three_colors(
		style.progress_bar_start_color,
		style.progress_bar_middle_color,
	...
	var display_text := _get_display_text(
		numeric_text,
		style.font,
		style.font_size,
		rect.size.x - absf(x_margin_val),
	)
	var text_width := style \
			.font \
			.get_string_size(display_text, HORIZONTAL_ALIGNMENT_LEFT, -1, style.font_size) \
			.x
	var text_pos := Vector2(
		rect.position.x + (rect.size.x - text_width) / 2.0,
		get_text_baseline_y(style.font, style.font_size, rect.position.y, rect.size.y),
	)
	var fill_width: float = maxf(
		0.001,
		fill.position.x + fill.size.x - text_pos.x - absf(x_margin_val) + 5 * scale,
	)
...
static func _get_display_text(text: String, font: Font, font_size: int, max_width: float) -> String:
	...
	for i in range(text.length()):
		...
		var test_width := font \
				.get_string_size(test_text, HORIZONTAL_ALIGNMENT_LEFT, -1, font_size) \
				.x
...
static func _get_interpolated_three_colors(
	start_color: Color,
	mid_color: Color,
	end_color: Color,
	progress: float,
...
static func _compute_drag_value(
	mouse_pos: Vector2,
	column: ColumnConfig,
	cell_x: float,
	cell_width: float,
...
static func _get_range_config(column: ColumnConfig) -> Dictionary:
	return column.get_cached(
		&"range_config",
		func() -> Dictionary:
			return _compute_range_config(column),
	)
